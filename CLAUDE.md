...
- **Recuperación de contraseña**: no hay correo. El camino normal son las **preguntas de seguridad** desde la propia app (`/api/auth/recuperar`). Como respaldo del dueño existen `MORSA_PASSWORD_RESET` y `apps/backend/reset_password.py`. Ver [docs/recuperar-password.md](docs/recuperar-password.md).
- **`_PgCursorWrapper` debe imitar a `sqlite3.Cursor`**. Si `database.py` usa un atributo que el wrapper no expone (`rowcount`, por ejemplo), el código funciona en los tests con SQLite y falla solo en PostgreSQL, que es producción.
